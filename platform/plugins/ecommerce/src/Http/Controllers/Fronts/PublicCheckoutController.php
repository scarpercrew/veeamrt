...
class PublicCheckoutController
{
    ...
    public function postCheckout(
        ...
        if ($order) {
            ...
            foreach (Cart::instance('cart')->content() as $cartItem) {
                ...
                $data['sub_total'] = $cartItem->price * $cartItem->qty;
                $data['tax'] = $data['tax_amount'] * $cartItem->qty;
        ...
        if ($request->input('address.address_id')) {
            ...
            $cartItems = $addresses[0]->toArray();
        } else {
            $cartItems['name'] = $request->input('address.name');
            $cartItems['email'] = $request->input('address.email');
            $cartItems['phone'] = $request->input('address.phone');
            $cartItems['country'] = $request->input('address.country');
            $cartItems['state'] = $request->input('address.state');
            $cartItems['city'] = $request->input('address.city');
            $cartItems['address'] = $request->input('address.address');
            $cartItems['customer_id'] = $request->input('address.customer_id');
            $cartItems['is_default'] = $request->input('address.is_default');
            $cartItems['created_at'] = $request->input('address.created_at');
            $cartItems['updated_at'] = $request->input('address.updated_at');
            $cartItems['zip_code'] = $request->input('address.zip_code');
        }
        $cartItems['products'] = $listCarts;
        $cartItems['sub_total'] = $request->input('sub_total');
        $cartItems['tax_amount'] = $request->input('tax_amount');
        $cartItems['amount'] = $request->input('amount');
        $cartItems['order_id'] = $request->input('order_id');

        $_whatsapp = "https://wa.me/+919486700110?text=";
        $whatsapp = "Order ID: " . $cartItems['order_id'] . "\n";
        $whatsapp .= "Name: " . $cartItems['name'] . "\n";
        $whatsapp .= "Phone: " . $cartItems['phone'] . "\n\n";
        $whatsapp .= "Product Detail(s):\n\n";
        foreach ($cartItems['products'] as $product) {
            $whatsapp .= $product['product_id'] . ". ";
            $whatsapp .= $product['product_name'] . " - ";
            $whatsapp .= "Qty(" . $product['qty'] . ") - ";
            $whatsapp .= "₹(" . $product['total'] . ")\n\n";
        }
        $whatsapp .= "Sub Total: " . $cartItems['sub_total'] . "\n";
        $whatsapp .= "Tax Amount: " . $cartItems['tax_amount'] . "\n";
        $whatsapp .= "Total: " . $cartItems['amount'] . "\n";
        $_whatsapp .= urlencode($whatsapp);
        // return redirect()->to($_whatsapp)->with('_blank');
        return redirect()->away($_whatsapp)->with('_blank');
        // $this->sendWhatsapp($cartItems);
    }
    ...
    public function sendWhatsapp(
        ...
        $_whatsapp = "https://wa.me/+919486700110?text=";
        $whatsapp = "Order ID: " . $data['order_id'] . "\n";
        ...
        $whatsapp .= "Sub Total: " . $data['sub_total'] . "\n";
        $whatsapp .= "Tax Amount: " . $data['tax_amount'] . "\n";
        $whatsapp .= "Total: " . $data['amount'] . "\n";
        $_whatsapp .= urlencode($whatsapp);

        // return Redirect::to($url);
        // echo $_whatsapp;
        // header("Location: ".$_whatsapp);
        // return redirect('https://www.google.com');
        // return $response->setNextUrl($_whatsapp)->with('_blank');
        // return redirect()->away('https://www.google.com');
        // return redirect()->away($_whatsapp)->with('_blank');
        return redirect()->to('https://wa.me/+919486700110?text=Order+ID%3A+70%0AName%3A+Rajaram+RM%0APhone%3A+8122400663%0A%0AProduct+Detail%28s%29%3A%0A%0A30.+Beat+Headphone+-+Qty%286%29+-+%E2%82%B9%28132%29%0A%0A60.+Sound+Intone+I65+Earphone+White+Version+-+Qty%282%29+-+%E2%82%B9%281188%29%0A%0ASub+Total%3A+1200%0ATax+Amount%3A+120%0ATotal%3A+1320%0A')->with('_blank');

        // echo "<pre>postCheckout(1): "; print_r($data); echo "</pre>";
        // exit;
        // this.cartItem.map((product: any) => {
        // $whatsapp .= product.ID . '. '
        // $whatsapp .= product.NAME . ' - ';
        // $whatsapp .= 'Qty(' . product.QTY . ') - ';
        // $whatsapp .= 'Rs(' . product.PRICE . ')%0a%0a';
        // });
